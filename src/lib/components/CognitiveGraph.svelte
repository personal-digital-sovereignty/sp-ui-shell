	...
	onMount(async () => {
		...
		Graph = ForceGraph3D()(container)
			...
			.linkDirectionalParticleColor((link: any) =>
				getNodeColor(link.source?.name || link.source || 'x')
			)
...
{#if topCategories.length > 0}
	<div
		class="pointer-events-none absolute left-6 top-6 w-60 bg-[#050510]/50 backdrop-blur-xl rounded-xl border border-slate-700/50 p-5 shadow-[0_0_30px_rgba(0,255,255,0.05)] z-10 transition-all text-white"
	>
		<div class="flex items-center gap-2 mb-4 border-b border-slate-700/50 pb-3">
			...
			<h3
				class="text-[11px] font-mono tracking-widest text-slate-300 font-bold uppercase shadow-sm"
			>
				Vault Taxonomy
			</h3>
		</div>
				...
				<li class="flex items-center justify-between text-xs font-mono">
					<div class="flex items-center gap-2.5">
						<span
							...
							style="background-color: {cat.color}; color: {cat.color};"
						></span>
