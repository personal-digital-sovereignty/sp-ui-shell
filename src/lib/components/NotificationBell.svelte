...
<div class="relative" use:clickOutside={() => isOpen = false}>
    ...
    {#if isOpen}
        <div 
            ...
            class="absolute right-0 top-12 w-80 bg-white border border-slate-200 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] z-[9999] overflow-hidden flex flex-col max-h-[400px]"
        >
            <div class="px-4 py-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center z-10 sticky top-0">
                <h3 class="text-sm font-bold text-slate-800">Alertas Cíbridos</h3>
                {#if unreadCount > 0}
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
                {#if globalState.notifications.length === 0}
                    <div class="p-8 text-center flex flex-col items-center justify-center text-slate-400">
                        <CheckCircle2 class="w-8 h-8 text-slate-200 mb-2" />
                ...
                {:else}
                    {#each globalState.notifications as notif}
                        <div class="group relative px-4 py-3 border-b border-slate-50 last:border-0 hover:bg-indigo-50/30 transition-colors flex gap-3 {notif.read ? 'opacity-70' : ''}">
                            <div class="mt-1">
                            ...
                            {#if notif.link}
                                <a href={notif.link} onclick={() => isOpen = false} class="flex-1 min-w-0 pr-6 cursor-pointer group/link block focus:outline-none focus:ring-2 focus:ring-indigo-500/20 rounded-md">
                                    <div class="flex items-center justify-between mb-0.5">
                                        <span class="text-xs font-bold text-slate-800 truncate" class:text-indigo-900={!notif.read}>{notif.title}</span>
                                        <span class="text-[9px] text-slate-400 font-mono shrink-0 ml-2">{notif.time}</span>
                                    </div>
                                    <p class="text-[11px] text-slate-500 leading-snug line-clamp-2">{notif.text}</p>
