...
{#if isOpen}
	...
	<div
		class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 sm:p-6 animate-in fade-in duration-200 print-modal-wrapper"
	>
		<!-- Click outside to close -->
		<div
			class="absolute inset-0 print-hidden"
		...
		<div
			class="relative bg-white dark:bg-[#12192b] w-full max-w-4xl max-h-[85vh] rounded-2xl shadow-2xl flex flex-col border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200 print-modal-content"
		>
			<!-- Header -->
			<div
				class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/80 dark:bg-[#0c1324] shrink-0 print-hidden"
			>
				<div class="flex items-center gap-3">
					<div
						class="p-2 bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 rounded-lg shadow-sm"
					>
				...
				<div class="flex items-center gap-3">
					<button
						onclick={printManual}
						class="px-3 py-1.5 rounded-lg bg-emerald-600 text-white hover:bg-emerald-500 transition-colors cursor-pointer flex items-center gap-2 shadow-sm font-medium"
						title="Export to PDF"
					>
