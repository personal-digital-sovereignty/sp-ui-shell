	...
	$effect(() => {
		if (project) {
			chatLog = [
				{
					role: 'assistant',
					content: `Olá! Sou seu Sovereign Pair. Estou vendo o projeto **${project.name}** com suas ${project.tasks?.length || 0} tarefas. Como posso ajudar a acelerar essa entrega hoje?`
				}
			];
		}
	});
...
{#if !isOpen}
	<button
		onclick={() => (isOpen = true)}
		class="fixed bottom-6 right-8 bg-emerald-600 dark:bg-emerald-500 text-white p-3.5 rounded-full shadow-lg hover:bg-emerald-700 dark:hover:bg-emerald-600 transition z-50 flex items-center justify-center cursor-pointer group hover:scale-110"
	>
		...
		<span
			class="absolute right-[110%] mr-2 max-w-0 overflow-hidden whitespace-nowrap group-hover:max-w-xs transition-all duration-300 ease-in-out font-bold text-sm bg-emerald-800 dark:bg-emerald-900 text-emerald-50 rounded-lg group-hover:px-3 group-hover:py-1.5 opacity-0 group-hover:opacity-100 shadow-sm"
		>
	...
	>
		...
		>
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p
						class="text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap overflow-hidden text-ellipsis max-w-[150px] transition-colors"
					>
						Lendo: {project.name}
					</p>
				</div>
				...
				>
					<div class="flex items-center gap-2 mb-1.5 px-1">
						...
						<span
							...
							>{msg.role === 'user' ? 'Commander' : 'Project AI'}</span
						>
					</div>
		...
		>
			...
			>
				...
				<button
					type="submit"
					class="absolute right-2 bottom-2 p-2.5 bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white rounded-lg transition-colors cursor-pointer"
				>
					<Send class="w-5 h-5" />
				</button>
			</form>
