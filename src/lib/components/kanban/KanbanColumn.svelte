...
<section class="w-80 flex flex-col rounded-xl border border-slate-200 shadow-sm max-h-full bg-slate-50 shrink-0 relative">
    <!-- Column Header -->
    <div class="p-4 flex items-center justify-between border-b border-slate-200/60 bg-white/50 rounded-t-xl shrink-0 group">
        <h2 class="font-semibold text-slate-800 text-lg truncate pr-2">{colStatus}</h2>
        <div class="flex items-center gap-2 relative">
            ...
            {#if isMenuOpen}
                <div class="absolute top-full right-0 mt-1 w-40 bg-white border border-slate-200 shadow-xl rounded-lg overflow-hidden z-30">
                    <button onclick={renameColumn} class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2 cursor-pointer">
                        <Edit2 class="w-3 h-3"/> Renomear
                    </button>
                    <button onclick={deleteColumn} class="w-full text-left px-4 py-2 text-sm text-rose-600 hover:bg-rose-50 flex items-center gap-2 cursor-pointer">
