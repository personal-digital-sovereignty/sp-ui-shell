<script lang="ts">
	...
	function parseMarkdown(text: string) {
		...
		let sanitizedMarkup = DOMPurify.sanitize(raw as string, {
	...
	let chatLog = $state<{ role: string; content: string }[]>([
		{
			role: 'assistant',
			content: `Bem-vindo ao Hub Master! Como posso te ajudar na orquestração hoje? (Ex: *'Criar um projeto para o Q4'*, *'Resumir tarefas travadas'*).`
		}
	]);
	...
	async function sendMessage() {
		...
		try {
							...
							const tasksSummary =
								p.tasks && p.tasks.length > 0
									? p.tasks
											.map(
												(t) =>
													`    * [${t.status}] ${t.title} (Criada: ${t.created_at ? t.created_at.split(' ')[0] : '?'})`
											)
											.join('\n')
			...
			const apiMessages = [{ role: 'system', content: systemContext }, ...chatLog.slice(0, -1)];
...
{#if !isOpen}
	<button
		onclick={() => (isOpen = true)}
		class="fixed bottom-6 right-8 bg-blue-600 dark:bg-blue-500 text-white p-3.5 rounded-full shadow-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition z-50 flex items-center justify-center cursor-pointer group hover:scale-110"
	>
		...
		<span
			class="absolute right-[110%] mr-2 max-w-0 overflow-hidden whitespace-nowrap group-hover:max-w-xs transition-all duration-300 ease-in-out font-bold text-sm bg-blue-800 dark:bg-blue-900 text-blue-50 rounded-lg group-hover:px-3 group-hover:py-1.5 opacity-0 group-hover:opacity-100 shadow-sm"
		>
			Orchestration AI
		</span>
...
{:else}
	<aside
		class="w-[450px] bg-slate-50 dark:bg-[#0c1324] flex flex-col h-full overflow-hidden shrink-0 border-l border-slate-200 dark:border-slate-800 font-sans z-10 transition-colors"
	>
		<!-- Chat Header -->
		<header
			class="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-[#12192b] shrink-0 transition-colors"
		>
			<div class="flex items-center gap-3">
				...
				<div>
					<h2 class="text-slate-800 dark:text-slate-200 font-bold tracking-tight transition-colors">
						Hub Master <span
							class="text-[10px] bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded ml-2 uppercase tracking-widest font-mono transition-colors"
							>Global Node</span
						>
					</h2>
					<p class="text-xs text-slate-500 dark:text-slate-400 transition-colors">
						{activeCount} Projetos Ativos no Hub.
					</p>
				</div>
			</div>
			<div class="flex items-center gap-2 text-slate-500">
				<div class="flex flex-col items-end mr-1 hidden sm:flex">
					<span
						class="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 transition-colors"
						>Sovereign Core</span
					>
					<span
				...
				<button
					...
					class="p-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
				>
					<X class="w-5 h-5" />
				</button>
			</div>
		...
		<!-- Message Feed -->
		<main
			class="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-6 custom-scrollbar bg-slate-50 dark:bg-[#0c1324] transition-colors"
		>
			{#each chatLog as msg}
				<div
					class="flex flex-col max-w-3xl group {msg.role === 'user'
						? 'self-end items-end'
				...
				>
					<div class="flex items-center gap-2 mb-1.5 px-1">
						{#if msg.role === 'assistant'}
							<div class="relative w-7 h-7 rounded-full flex items-center justify-center shrink-0">
								...
								{#if isTyping && msg === chatLog[chatLog.length - 1]}
									<div
										class="absolute inset-0 rounded-full animate-ping opacity-30 bg-blue-500"
									></div>
								{/if}
						...
						{:else}
							<div
								class="relative w-7 h-7 rounded-full bg-slate-200 dark:bg-slate-700/50 flex items-center justify-center shrink-0 p-1.5 text-slate-600 dark:text-slate-300"
							>
								<svg fill="none" stroke="currentColor" class="w-full h-full" viewBox="0 0 24 24"
									><path
						...
						{/if}
						<span
							class="text-[10px] uppercase font-bold tracking-widest {msg.role === 'user'
								? 'text-slate-500'
								: 'text-blue-600 dark:text-blue-400'}"
							>{msg.role === 'user' ? 'Commander' : 'Hub Master'}</span
						>
					</div>

					<div
						class="{msg.role === 'user'
							? 'bg-slate-200 dark:bg-slate-700/80 text-slate-800 dark:text-slate-200 rounded-2xl rounded-tr-none'
							: 'bg-white dark:bg-[#12192b] border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.1)] dark:shadow-none rounded-2xl rounded-tl-none'} p-4 text-[14px] leading-relaxed prose dark:prose-invert max-w-full overflow-x-auto custom-scrollbar transition-colors"
					>
						{@html parseMarkdown(msg.content)}
					</div>

					<div
						class="flex items-center gap-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 {msg.role ===
						'user'
							? 'mr-1 justify-end'
							: 'ml-1 justify-start'}"
					>
						<button
							class="p-1.5 rounded-lg bg-white dark:bg-[#12192b] hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 border border-slate-200 dark:border-slate-800 transition-all cursor-pointer"
							><Copy class="w-3.5 h-3.5" /></button
						>
						{#if msg.role === 'user'}
							<button
								class="p-1.5 rounded-lg bg-white dark:bg-[#12192b] hover:bg-blue-50 dark:hover:bg-blue-900/30 hover:text-blue-600 dark:hover:text-blue-400 text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-slate-800 hover:border-blue-200 dark:hover:border-blue-800/50 transition-all cursor-pointer"
								><RotateCcw class="w-3.5 h-3.5" /></button
							>
						{/if}
						{#if msg.role === 'assistant'}
							<button
								class="p-1.5 rounded-lg bg-white dark:bg-[#12192b] hover:bg-emerald-50 dark:hover:bg-emerald-900/40 text-slate-400 dark:text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-400 border border-slate-200 dark:border-slate-800 hover:border-emerald-200 dark:hover:border-emerald-800 transition-all cursor-pointer"
								><ThumbsUp class="w-3.5 h-3.5" /></button
							>
							<button
								class="p-1.5 rounded-lg bg-white dark:bg-[#12192b] hover:bg-rose-50 dark:hover:bg-rose-900/30 text-slate-400 dark:text-slate-500 hover:text-rose-600 dark:hover:text-rose-400 border border-slate-200 dark:border-slate-800 hover:border-rose-200 dark:hover:border-rose-800 transition-all cursor-pointer"
								><ThumbsDown class="w-3.5 h-3.5" /></button
							>
						{/if}
					</div>
				</div>
		...
		<!-- Input Bar -->
		<footer
			class="p-4 bg-white dark:bg-[#12192b] border-t border-slate-200 dark:border-slate-800 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)] transition-colors"
		>
			<form
				onsubmit={(e) => {
					e.preventDefault();
			...
			>
				...
				<button
					type="submit"
					disabled={isTyping}
					class="absolute right-2 bottom-2 p-2.5 bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white rounded-lg transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
				>
					{#if isTyping}
			...
			<div
				class="text-[10px] text-center text-slate-400 dark:text-slate-500 mt-2 font-mono uppercase tracking-widest transition-colors"
			>
