<script lang="ts">
	...
	function connectSSE() {
		...
		eventSource.onmessage = (event) => {
			...
			try {
				...
				if (telemetryState.systemLogs.length > 1000)
					telemetryState.systemLogs = telemetryState.systemLogs.slice(
						telemetryState.systemLogs.length - 1000
					);
	...
	function downloadLogs() {
		...
		const rawText = telemetryState.systemLogs
			.map((l: LogEntry) => `[${l.timestamp}] ${l.level.toUpperCase()} - ${l.message}`)
			.join('\n');
...
<div
	class="flex flex-col h-full bg-surface-900 border border-surface-700/50 rounded-2xl overflow-hidden shadow-2xl"
>
	<!-- Header -->
	<div
		class="bg-surface-800/80 backdrop-blur-md px-6 py-4 border-b border-surface-700/50 flex justify-between items-center z-10 shrink-0"
	>
		<div class="flex items-center gap-3">
			...
			<div>
				<h1
					class="text-xl font-bold text-surface-100 font-syncopate tracking-widest flex items-center gap-2"
				>
					System Logs
					{#if !isPaused}
						<span
							class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_theme(colors.emerald.500)]"
						></span>
					{:else}
		...
		<div class="flex items-center gap-2">
			<button
				onclick={togglePause}
				class="btn-icon {isPaused
					? 'bg-amber-500/10 text-amber-400'
					: 'bg-surface-700 text-surface-300'} px-3 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 hover:bg-surface-600 transition-colors"
			...
			<button
				onclick={clearLogs}
				class="btn-icon bg-surface-700 text-surface-300 px-3 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 hover:bg-rose-500/20 hover:text-rose-400 transition-colors"
				title="Clear Buffer"
			>
			...
			<button
				...
				class="btn-primary bg-primary-500/10 text-primary-400 border border-primary-500/30 px-3 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 hover:bg-primary-500/20 transition-all shadow-[0_0_15px_theme(colors.primary.500/20)]"
				title="Export Full Session to File for Debugging"
			>
	...
	>
		{#if telemetryState.systemLogs.length === 0}
			<div
				class="h-full flex flex-col items-center justify-center text-surface-500 gap-4 opacity-50"
			>
		...
		{:else}
			{#each telemetryState.systemLogs as log}
				<div
					class="flex items-start gap-3 hover:bg-surface-800/30 px-2 py-1 rounded transition-colors group break-words w-full"
				>
					<div
						class="flex-shrink-0 text-surface-500 mt-0.5 w-[75px] truncate text-xs flex items-center gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity"
					>
						<Clock class="w-3 h-3" />
						{log.timestamp}
					</div>
					...
					<div
						class="flex-shrink-0 w-16 uppercase text-[10px] font-bold tracking-widest text-center border rounded mt-0.5 {getLevelColor(
							log.level
						)}"
					>
						{log.level}
