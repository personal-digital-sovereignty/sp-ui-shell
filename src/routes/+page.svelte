...
<div class="flex flex-col h-full bg-slate-50 w-full overflow-hidden">
	<!-- Header -->
	<header
		class="h-16 border-b border-slate-200 bg-white flex items-center px-6 shrink-0 justify-between shadow-sm z-10"
	>
		<div class="flex items-center gap-3">
			...
			<span
				class="px-2.5 py-1 bg-slate-100 text-slate-600 text-[10px] uppercase tracking-widest font-bold rounded-full ml-2 border border-slate-200"
				>Cognitive Graph</span
			>
		</div>

		<div class="flex items-center gap-4">
			<div
				class="flex items-center gap-5 text-sm font-semibold text-slate-600 bg-slate-50 px-5 py-2 rounded-xl border border-slate-200 shadow-inner"
			>
	...
	<main class="flex-1 relative p-6 flex flex-col">
		<div
			class="w-full flex-1 rounded-2xl overflow-hidden shadow-2xl border border-slate-800/50 bg-[#0f172a] relative flex items-center justify-center"
		>
			{#if loading}
			...
			{:else if error}
				<div
					class="flex flex-col items-center gap-2 text-red-400 p-6 bg-red-950/30 rounded-xl border border-red-900/50"
				>
			...
			{:else}
				...
				<div
					class="absolute bottom-8 left-1/2 -translate-x-1/2 bg-slate-800/80 backdrop-blur-xl border border-slate-600/50 px-8 py-5 rounded-2xl flex items-center gap-8 shadow-[0_8px_30px_rgb(0,0,0,0.5)]"
				>
					<div class="flex flex-col gap-3">
						<label
							for="graph-depth"
							class="text-xs font-bold text-slate-400 uppercase tracking-widest flex justify-between"
						>
						...
						<input
							id="graph-depth"
							type="range"
							class="w-48 accent-blue-500 cursor-pointer"
							min="1"
							max="5"
							value="5"
						/>
					...
					<div class="flex flex-col gap-3">
						...
						<input
							id="data-history"
							type="range"
							class="w-48 accent-emerald-500 cursor-pointer"
							min="0"
							max="100"
							value="100"
						/>
