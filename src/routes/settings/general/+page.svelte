<script lang="ts">
	...
	let offlineCorpora = $state([
		{
			id: 'fineweb',
			name: 'FineWeb / FineWeb-Edu (Parquet)',
			size: '2.4 TB',
			active: false,
			priority: 1,
			type: 'HuggingFace'
		...
		{
			id: 'zim-wiki',
			name: 'Wikipedia Integral (Kiwix ZIM)',
			size: '98 GB',
			active: true,
			priority: 4,
			type: 'ZIM Dump'
	...
	const PREDEFINED_PERSONAS = [
		...
		{
			label: '2. The Code Architect (Engenheiro Sênior)',
			prompt:
				'Você é O Arquiteto de Código. Forneça código impecável, idiomático e altamente documentado.'
		},
		{
			label: '3. The Security Auditor (DevSecOps)',
			prompt: 'Você é um Auditor de Segurança implacável.'
		},
		{
			label: '4. The Financial Analyst (Quant)',
			prompt: 'Você é um analista financeiro quantitativo.'
		}
	];
	...
	onMount(async () => {
		...
		try {
			...
			if (csRes.ok) {
				...
				if (data.offlineCorpora && data.offlineCorpora.length > 0)
					offlineCorpora = data.offlineCorpora;
			}
		} catch (e) {}
	});
	...
	async function handleFileImport(e: Event) {
		...
		reader.onload = async (e) => {
			...
			try {
				...
				if (postRes.ok) {
					alert('Config imported! Restart the engine.');
					location.reload();
				}
			} catch (err) {
				alert('Invalid .cybrid file.');
			}
		};
					...
					<div class="flex items-center gap-4 mb-8 pb-6 border-b border-outline-variant/10">
						...
						<div>
							...
							<p class="text-[11px] font-medium text-on-surface-variant mt-1">
								Master user preferences, startup page, and configuration backup.
							</p>
						</div>
					</div>
					...
					<div class="space-y-6">
						...
						<div class="flex flex-col gap-1.5">
							<label
								...
								>Startup Landing Page</label
							>
							<select
								id="default_route"
								bind:value={aiSettings.default_route}
							...
							<p class="text-[10px] text-on-surface-variant/70 font-medium">
								Sovereign Pair will boot directly to this page.
							</p>
						</div>
						...
						<div class="flex flex-col gap-1.5">
							...
							<textarea
								id="system_prompt"
								bind:value={aiSettings.system_prompt}
								rows="4"
								placeholder="Leave empty for default Sovereign Practitioner..."
								class="w-full bg-surface-container-high border border-outline-variant/30 rounded-2xl p-4 text-sm font-medium text-on-surface placeholder:text-on-surface-variant/40 focus:ring-2 focus:ring-primary outline-none resize-none custom-scrollbar"
							></textarea>
						</div>
						...
						<div class="flex items-center gap-6">
							<div class="flex-1 flex flex-col gap-1.5">
								<label class="text-xs font-bold text-on-surface-variant uppercase tracking-widest"
									>Temperature: {aiSettings.temperature}</label
								>
								<input
									type="range"
									min="0"
			...
			<div class="col-span-12 xl:col-span-6 space-y-6">
				...
				>
					<div class="relative z-10">
						...
						<p class="text-xs text-white/70 mb-8 font-medium leading-relaxed">
							A `.cybrid` file contains fully encrypted O.S State (Workspaces, Mesh Keys,
							Preferences) to instantly synchronize a new node.
						</p>
						...
						<div class="flex gap-3">
							<button
								...
								class="flex-1 py-3 rounded-xl bg-white/10 text-white hover:bg-white/20 transition-colors flex items-center justify-center gap-2 cursor-pointer border border-white/20 text-sm font-bold"
							>
								<Upload class="w-4 h-4" /> Import
							</button>
							<a
								href="{API_BASE_URL}/v1/system/export_config"
								target="_blank"
								class="flex-1 py-3 rounded-xl bg-white text-[#001360] hover:bg-white/90 transition-colors flex items-center justify-center gap-2 cursor-pointer text-sm font-bold shadow-md"
							>
